...
def chequepdf(request, id):
    ...
    if cheque.party:
        p.drawString(-210 * mm, 70 * mm, f'{cheque.party}')
    p.drawString(-205 * mm, 60 * mm, f'{cheque.amountinword}')
    if cheque.amountint:
        p.drawCentredString(-35 * mm, 50 * mm, f'**{cheque.amountint}**')
    t = p.beginText()
    if cheque.bank.id==1:
        t.setTextOrigin(-166, 88 * mm)
        t.setCharSpace(11.5)
    else:
        t.setTextOrigin(-154, 88 * mm)
        t.setCharSpace(10.5)
