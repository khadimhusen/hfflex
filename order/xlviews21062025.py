...
def processlist(request, status=None):
    ...
    columns = ['job', 'process', 'qty', 'unit', 'pouch qty', 'status', 'size','cylinder']
    col_width = {"A": 100, "B": 10, "C": 10, "D": 10, "E": 10, "F": 10, "G": 10, "H":10}
    ...
    for process in process_queryset:
        row_num += 1
        row = [str(process.job), str(process.process), process.qty, str(process.unit), process.job.totalpouch,
