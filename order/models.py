...
class Job(models.Model):
        ...
        if self.jobprocess.filter(process__process="Printing").exists():
            ...
            if printingfilm:
                printwaste = round(float(printingfilm.gsm) * float(printingfilm.size) *
                                   0.0005*(1+float(self.totalmeter or 0)/15000) + 2, 2)
    ...
    def std_waste_percentage(self):
        if self.kgqty:
            return round(float(self.std_waste_kg or 0) * 100 / (float(self.kgqty) + float(self.std_waste_kg)), 2)
        else:
            return 0
    ...
    def _production_cost_summary(self):
        from production.models import ProdInput, Stockdetail, ProdReport
        from django.contrib.contenttypes.models import ContentType

        report_ids = list(ProdReport.objects.filter(
            prodprocess__job=self
        ).values_list('id', flat=True))

        # Build the same inputdetail dict as the view
        inputdetail = {}

        # --- inputs (consumed materials) ---
        inputs = ProdInput.objects.filter(
            prodreport__in=report_ids
        ).select_related('material__materialname', 'material__item_mat_type', 'material__item_grade')

        for inp in inputs:
            key = inp.material.mate_name
            qty = round((-inp.wtgain or 0), 3)
            amount = round(((inp.material.rate or 0) * (inp.inputqty or 0)), 3)
            if key not in inputdetail:
                inputdetail[key] = {"qty": qty, "amount": amount}
            else:
                inputdetail[key]["qty"] += qty
                inputdetail[key]["amount"] += amount

        # --- outputs (produced rolls) ---
        ct = ContentType.objects.get_for_model(ProdReport)
        outputs = Stockdetail.objects.filter(
            content_type=ct,
            object_id__in=report_ids,
        ).select_related('materialname', 'item_mat_type', 'item_grade')

        for out in outputs:
            key = out.mate_name
            qty = round((out.recieved or 0), 3)
            if key not in inputdetail:
                inputdetail[key] = {"qty": qty, "amount": 0}
            else:
                inputdetail[key]["qty"] += qty  # cancels against input if same material

        # --- summarise, same logic as view ---
        cost = 0
        netoutput = 0
        for key, val in inputdetail.items():
            if val["qty"] < -0.0001:  # net consumed → external raw material
                cost += val["amount"]
            elif val["qty"] > 0.0001:  # net produced → finished output
                if 'WASTE' not in key:
    ...
    @property
    def cost(self):
        cost, _ = self._production_cost_summary()
        return cost

    @property
    def salecost(self):
        _, netoutput = self._production_cost_summary()
        return round(netoutput * self.kgrate, 3)

    @property
    def profit(self):
        cost, netoutput = self._production_cost_summary()
        salecost = round(netoutput * self.kgrate, 3)
        difference = round(salecost - cost, 3)
        per_kg = round(difference / netoutput, 3) if netoutput else 0
        return {"difference": difference, "per_kg": per_kg}
