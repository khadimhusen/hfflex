...
from preorder.forms import AddPreOrderForm, RemovePreOrderForm
...
def editpreorder(request, id=None):
    ...
    removepreorderform = RemovePreOrderForm(instance=preorder)
    ...
    context['removepreorderform'] = removepreorderform
...
@login_required(login_url='/login/')
@accessview
def removefinalsubmition(request, id=None):
    preorder = get_object_or_404(PreOrder, id=id)
    removepreorderform = RemovePreOrderForm(request.POST or None, instance=preorder)

    if request.method == 'POST':
        if removepreorderform.is_valid():
            removepreorderform.save()
            return HttpResponseRedirect(reverse('preorder:editpreorder', kwargs={'id': preorder.id}))
        else:
            return HttpResponseRedirect(reverse('preorder:editpreorder', kwargs={'id': preorder.id}))
