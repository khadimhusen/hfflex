...
class Shift(models.Model):
    ...
    def totaldowntime(self):
        result = Activity.objects.filter(shift=self).aggregate(Sum('downtime'))
        return round(result['downtime__sum'] or 0,0)
...
class Activity(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.makereadytime = TimeMaster.objects.filter(machine=self.shift.machine,
                                                       makeready=self.makeready)[0].allotedtime or 0
        if self.speed == 0:
            self.totaltime = 0
        else:

            self.totaltime = round(
                (self.qty / self.speed) + self.makereadytime + (self.rolls * 5) + (self.lot * 10) + self.downtime, 0)
