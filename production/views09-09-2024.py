...
def dispatchdetailedit(request, id=None):
    ...
    if request.method == 'POST':
        dispatch_form = DispatchForm(request.POST, instance=disp)
        context['dispatch_form'] = dispatch_form
        if dispatch_form.is_valid():
            dispatch = dispatch_form.save(commit=False)
...
def dispatchpending(request):
    ...
    for material in material_list:
        ...
        if not cust in obj:
            jobs = {}
            jobs[jobname] = {'qty': material.recieved,'nos':material.nos or 0, 'orderqty': orderqty, 'remark': jobname.dispatch_remark}
            obj[cust] = jobs
            ...
            if jobname in obj[cust]:
                ...
                obj[cust][jobname] = {'qty': material.recieved,'nos':material.nos or 0, 'orderqty': orderqty, 'remark': jobname.dispatch_remark}
