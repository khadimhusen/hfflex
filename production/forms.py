...
class InwardForm(forms.ModelForm):
    ...
    class Meta:
        model = Inward
        fields = ['docdate', 'supplier', 'inwarddate','invoice']
...
class ProdReportForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(ProdReportForm, self).__init__(*args, **kwargs)
        self.fields['supervisor'].queryset = User.objects.filter(department__department_name="supervisor")

        if 'supervisor' in self.data:
            try:
                print("self data",self.data)
                self.fields['supervisor'].queryset = self.fields['supervisor'].queryset = User.objects.filter(department__department_name="supervisor")


            except (ValueError, TypeError):
                pass  # invalid input from the client; ignore and fallback to empty City queryset
        elif self.instance.pk:
            query=User.objects.filter(id=self.instance.supervisor_id) | User.objects.filter(department__department_name="supervisor")
            self.fields['supervisor'].queryset = query.distinct()
...
class NewProdReportForm(forms.ModelForm):
    ...
    class Meta:
        model = ProdReport
        fields = ["processdate","qty","unit","totalkg","supervisor"]
